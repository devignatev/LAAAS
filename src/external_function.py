...
def individuals_sorted(df, nums_grandchild):
    dft = df.transpose()
    dict_data = dft.to_dict()
    return_data = {}
    add_person = []

    for human in list(dict_data):
        if len(human.split('+')) > 3:

            grandchild = human
            # Превращаем это (YAK85+YAK86-5)+(YAK75+YAK76-2)—4 в это ['YAK85', 'YAK86-5', 'YAK75', 'YAK76-2']
            human_list = human.replace('(', '')
            human_list = human_list.replace(')', '')
            human_list = human_list.split('+')
            human_list[3] = (human_list[3].split('—'))[0]

            return_data[human] = dict_data[human]
            parents_list = [f"{human_list[0]}+{human_list[1]}", f"{human_list[2]}+{human_list[3]}"]

            grandparents_list = [human_list[0], (human_list[1].split('-'))[0], human_list[2], (human_list[3].split('-'))[0]]

            family = []
            family = family + parents_list + grandparents_list

            # Добавим не только (YAK85+YAK86-5)+(YAK75+YAK76-2)—1, но и всех остальных детей,
            # например (YAK85+YAK86-5)+(YAK75+YAK76-2)—2
            grandchild = grandchild.split('—')[0]
            for i in range(1, nums_grandchild + 1):
                person = f"{grandchild}—{i}"
                if person not in add_person:
                    add_person.append(person)
                    return_data[person] = dict_data[person]

            #Добавим всю остальную семью
